  ...
  if ( peek( Token::left_brace ) )
    for ( ; ; )
    {
      if ( peek( "eye" ) )
        ...
        hfov = parseReal();
      else if ( peek( "lens_radius" ) )
        rad = parseReal();
      else if ( peek( "view_plane_distance" ) )
        viewd = parseReal();
      else if ( peek( "focal_point" ) )
        fpoint = parseReal();
      else if ( peek( Token::right_brace ) )
        break;
      else
        throwParseException( "Expected `eye', `lookat', `up', `hfov', `lens_radius', `view_plane_distance', `focal_point' or }." );
    }
  return new DOFCamera( eye, lookat, up, hfov, rad, viewd, fpoint );
}
...
Camera *Parser::parseCamera()
{
    ...
    if ( peek( "dof" ) )
        return parseDOFCamera();
    throwParseException( "Expected `pionhole' or `dof' type." );
    return 0;
}
  ...
  if ( peek( Token::left_brace ) )
    for ( ; ; )
    {
      if ( peek( "color" ) )
        ...
        throwParseException( "Expected `color', `Kd', `Ka', `Kr', `Ks' or }." );
    }
  return new LambertianMaterial( color, Kd, Ka, Kr, Ks );
}
...
Object *Parser::parsePlaneObject()
{
  ...
  if ( peek( Token::left_brace ) )
	...
	width = parseReal();
        ...
        throwParseException( "Expected `material', `point', `normal', 'width', or }." );
    }
  return new Plane( material, normal, point, width);
}
...
Object *Parser::parseMovingSphereObject()
{
	Material *material = default_material;
	Point center(0.0, 0.0, 0.0);
	double radius = 0.5;
	...
	if (peek( Token::left_brace ) ) {
		for ( ; ; )
		{
			if ( peek( "material" ) )
				...
				throwParseException( "Expected `material', `center', `radius', `speed', `direction' or }." );
		}
	}
	
	return new MovingSphere(material, center, radius, speed, direction);
	///////////////////////////////////////////////////////
}
...
Object *Parser::parseObject()
{
    ...
    else if ( peek( "moving_sphere"))
	return parseMovingSphereObject();
    else if ( next_token.token_type == Token::string )
    {
...
Parser::Parser(
  ...
  : input( input ),
    ...
    column_number( 0 )
{
	Color defColor = Color(1.0,1.0,1.0);
	default_material = new LambertianMaterial( defColor, 0.6, 0.3, 0.0, 10000.0 );
  readNextToken();
}
  ...
  string &filename )
{
  ...
  for ( ; ; ) {
    if ( peek( "filename" ) )
      ...
      yres = parseInteger();
    else if ( peek( "shutter_time"))
      shutterTime = parseReal();
    else if ( peek( "sample_size" ) )
      ss = parseInteger();
  ...
  scene->setShutterTime(shutterTime);
  scene->setSampleSize(ss);
  return scene;
}
